        ...
        container.insertAdjacentHTML('beforeend', `
            <div class="flex items-center justify-between bg-gray-900/50 p-3 rounded-2xl border border-white/5 animate-fade-in mb-2">
                <div class="flex items-center gap-3">
                    ${avatarHtml}
                    <div class="text-left"><p class="text-sm font-bold text-white">${p.username}</p><p class="text-[10px] text-gray-500 truncate w-24">${p.status_text || 'En ligne'}</p></div>
                </div>
                <div class="flex gap-2">
                    <button onclick="openDirectChat('${p.id}', '${p.username}')" class="p-2 bg-purple-600/20 text-purple-400 rounded-xl hover:bg-purple-600 hover:text-white transition-all"><i data-lucide="message-circle" class="w-4 h-4"></i></button>
                    <button onclick="removeFriend('${p.id}')" class="p-2 bg-red-600/10 text-red-400 rounded-xl hover:bg-red-600 hover:text-white transition-all"><i data-lucide="user-minus" class="w-4 h-4"></i></button>
                </div>
            </div>`);
    });
    if(typeof lucide !== 'undefined') lucide.createIcons();
...
async function loadConversations() {
    ...
    if(ids.length > 0) {
        ...
        if(profiles) profiles.forEach(p => { 
            ...
            if(c) { c.name = p.username; c.initials = p.username.substring(0,2).toUpperCase(); c.avatar_url = p.avatar_url; }
        });
    }
...
async function fetchLiveMessages() {
    ...
    } else { container.innerHTML = '<div class="text-center text-gray-600 text-xs py-10 italic">Bienvenue dans le chat du direct !</div>'; }
}
...
async function toggleComments(postId) {
    ...
    if (!section.classList.contains('hidden')) {
        ...
        if(comments && comments.length > 0) {
            comments.forEach(c => { list.innerHTML += `<div class="text-xs text-gray-300"><span class="font-bold text-purple-400">${c.user_name}:</span> ${c.content}</div>`; });
        } else { list.innerHTML = '<div class="text-[10px] text-gray-500 italic">Aucun commentaire.</div>'; }
    }
}
...
async function sendComment(postId) {
    ...
    await supabaseClient.from('comments').insert([{ post_id: postId, user_id: currentUser.id, user_name: userProfile.username, content: content }]);
    input.value = '';
...
async function fetchNotifications() {
    ...
    if (requests && requests.length > 0) {
        ...
        if(typeof lucide !== 'undefined') lucide.createIcons();
    } else { badge.classList.add('hidden'); if(list) list.innerHTML = '<div class="p-4 text-center text-xs text-gray-500">Rien pour le moment 🍃</div>'; }
}
...
async function addFriend(targetId) {
    ...
    if (!error) { btn.innerText = "Envoyé !"; btn.disabled = true; btn.className = "px-8 py-3 bg-gray-600 rounded-xl text-white font-bold text-sm cursor-default"; }
    else if (error.code === '23505') { alert("Invitation déjà envoyée !"); } else { console.error(error); }
}
...
async function uploadStory(input) {
    ...
    try {
        ...
        renderStoriesList();
    } catch (error) { alert("Erreur upload story: " + error.message); renderStoriesList(); }
}
        ...
        stories.forEach(s => {
            ...
            <div onclick="openStoryViewer('${storyData}')" class="flex flex-col items-center space-y-1 cursor-pointer shrink-0">
                <div class="w-14 h-14 rounded-full bg-gradient-to-tr from-pink-500 to-purple-600 p-[2px]">
                    <div class="w-full h-full rounded-full bg-gray-900 border-2 border-gray-900 overflow-hidden">${avatarContent}</div>
                </div>
...
function closeStoryViewer() {
    ...
    document.getElementById('story-viewer-image').src = ""; 
    if (currentStoryTimer) clearTimeout(currentStoryTimer);
}
