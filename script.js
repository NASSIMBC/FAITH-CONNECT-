...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && !e.shiftKey) {
        ...
        if (document.activeElement.id === 'reel-comment-input') {
            e.preventDefault();
            // sendReelComment(); // CORRECTION : Cette fonction n'est pas définie dans le script, je la commente pour éviter une erreur.
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (canvas) {
        ...
        // CORRECTION : On ne dessine plus automatiquement au démarrage pour éviter les erreurs.
        // La fonction drawCanvas sera appelée uniquement à l'ouverture de la modale.
...
// CORRECTION : Fonction drawCanvas sécurisée pour éviter les erreurs si des éléments HTML sont manquants.
function drawCanvas() {
    const canvas = document.getElementById('verse-canvas');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');

    // --- 1. RÉCUPÉRATION SÉCURISÉE DE TOUTES LES VALEURS ---
    const textInput = document.getElementById('verse-text-input');
    const text = textInput ? textInput.value : "Car Dieu a tant aimé le monde...";
    ...
    // Effets (valeurs par défaut si les éléments n'existent pas dans le HTML)
    ...
    // Filtres (valeurs par défaut si les éléments n'existent pas)
    ...
    if (text) {
        ...
        lines.forEach((line, i) => {
            let yPos = startY + (i * lineHeight) - (totalHeight / 2);
            if (strokeWidth > 0) {
